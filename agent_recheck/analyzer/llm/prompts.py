...
5. 如果文档没有问题或无法分析，返回空数组 []"""
    ...
    def get_analysis_prompt(cls) -> str:
        """获取文档分析提示词"""
        return """你是政府采购合规审查专家。请分析以下招标文件，识别明确的合规问题。

文件：{document_title}
内容：
{document_content}

【核心原则】只报告明确的、确凿的违规问题，宁缺毋滥。如果问题不明显或可合理解释，不要报告。

【检查范围与判断标准】

1. **非歧视性**（必须找到明确的歧视性表述）：
   - 报告："投标人须提供深圳市本地业绩"（明确地域歧视）
   - 不报告："需提供相关业绩证明"（正常业绩要求）
   - 必须是明确针对外地供应商的歧视性条款

2. **采购需求**：
   - 报告："设备参数等同于华为产品"（明确品牌倾向）
   - 不报告："需符合国家标准"（正常技术要求）
   - 占位符（如"序号X"）需在大段文字中出现，明显是未完成文档才报告

3. **价格评分**：
   - 报告："价格分权重为65%"（超过60%且明确）
   - 报告："未规定中小企业价格扣除"（明确缺失）
   - 不报告："价格分占一定比例"（未明确超标）

4. **资质要求**：
   - 报告："注册资本不低于5000万元"（明显过高）
   - 报告："须具备ISO14001认证"（非项目必需认证）
   - 不报告："具备相应资质证书"（模糊表述）

5. **合同条款**：
   - 报告："履约保证金为合同金额的20%"（明确超标）
   - 报告："违约金按每日3‰计算"（明确过高）
   - 不报告："按合同法规定执行"（正常引用）

6. **中小企业**：
   - 报告："未规定中小企业价格扣除政策"（明确缺失政策）
   - 不报告：非专门面向中小企业的项目

7. **逻辑错误**：
   - 报告：前后矛盾且影响理解的（如前后金额不一致）
   - 报告：联系方式完全缺失
   - 不报告：轻微格式问题
